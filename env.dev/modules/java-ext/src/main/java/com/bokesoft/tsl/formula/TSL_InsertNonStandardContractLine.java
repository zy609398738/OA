...
public class TSL_InsertNonStandardContractLine extends BaseMidFunctionImpl {
	...
	public Object evalImpl(String name, DefaultContext context, Object[] args, IExecutor arg3) throws Throwable {
		...
		DataTable detailTable = document.get("B_NonSTDContrTMPLDtl");
		...
		while (it.hasNext()) {
			...
			// 供应商
			String vendor_id = TypeConvertor.toString(headTable.getObject("CustomerID"));
			// 付款方式ID
			String terms_id = TypeConvertor.toString(headTable.getObject("PaymentTermID"));
			// 贸易条款
			String fob = TypeConvertor.toString(headTable.getObject("FOB"));
			// 创建日期
			...
			for (int rowIndex : list) {
				...
				currency_code = DictCacheUtil.getDictValue(context.getVE(), "Dict_Currency",
						TypeConvertor.toLong(detailTable.getObject(rowIndex, "Currency1")), "Code").toString();
