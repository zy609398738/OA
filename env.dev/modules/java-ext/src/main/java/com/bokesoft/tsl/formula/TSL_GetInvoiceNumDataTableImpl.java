...
public class TSL_GetInvoiceNumDataTableImpl extends BaseMidFunctionImpl {
	private String ACTION = "ERP_Invoice_Num_TO_BPM";
	...
	public Object evalImpl(String name, DefaultContext context, Object[] args, IExecutor executor) throws Throwable {
		...
		if (organization_id > 0) {
			jo = factory.createCondition("OU_ID", " = ", organization_id);
			ja.add(jo);
		}

		if (!supplier_number.isEmpty() && !supplier_number.equalsIgnoreCase("null")) {

			jo = factory.createCondition("SUP_NUM", " = ", supplier_number);
			ja.add(jo);
		}
