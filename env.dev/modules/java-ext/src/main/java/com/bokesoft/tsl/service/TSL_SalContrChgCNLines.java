...
public class TSL_SalContrChgCNLines implements IExtService2 {
	...
	public Object doCmd(DefaultContext context, Map<String, Object> args) throws Throwable {
		...
		DataTable headTable = document.get("B_SalContrChgCN");
		DataTable detailTable = document.get("B_SalContrChgCNDtl");
		...
		MetaTable detailMetaTable = metaDataObject.getMetaTable("B_SalContrChgCNDtl");
		...
		if (data instanceof JSONArray) {
			...
			for (int i = 0; i < reJSONArray.size(); ++i) {
				...
				detailTable.setObject("ModelDesc",
						TypeConvertor.toString(jsonObject.get("power")) + "+-+" + jsonObject.get("description"));
