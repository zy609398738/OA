...
public class EmailUtil {
	 ...
	 * @throws IOException
	 */
	public static boolean deletefile(String deletepath) throws FileNotFoundException, IOException {
		try {
			...
			if (!file.isDirectory()) {
				...
				for (int i = 0; i < filelist.length; i++) {
					...
					String filePath = new StringBuilder(deletepath).append(separator).append(filelist[i]).toString();
	...
	/**
	 * 根据名称得到邮箱的用户名 ex: 王绪豪<x_hwang@163.com>解析后得到 x_hwang@163.com
	 * 
	 * @param userNames名称
	 * @return 邮箱地址
	 */
		...
		if (mailNames.indexOf(",") != -1) {
			...
			for (String mailName : mailNameList) {
				if (mailName.indexOf(namePreSign) != -1 && mailName.indexOf(nameEndSign) != -1) {
					String mailAddress = mailName.substring(mailName.indexOf(namePreSign) + 1,
							mailName.indexOf(nameEndSign));
					mailAddressList.append(mailAddress).append(",");
				} else {
					mailAddressList.append(mailName).append(",");
				}
			}
			return mailAddressList.substring(0, mailAddressList.length() - 1).toString();
		} else {
			if (mailNames.indexOf(namePreSign) != -1 && mailNames.indexOf(nameEndSign) != -1) {
				String mailAddress = mailNames.substring(mailNames.indexOf(namePreSign) + 1,
						mailNames.indexOf(nameEndSign));
