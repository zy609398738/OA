...
public class TSL_GetContractBarcodeDataTableImpl extends BaseMidFunctionImpl {
	private String ACTION = "ERP_Sales_Contract_Number_TO_BPM";
	...
	public Object evalImpl(String name, DefaultContext context, Object[] args, IExecutor executor) throws Throwable {
		...
		long customerID = TypeConvertor.toLong(context.getPara("CustomerID"));
		...
		if (customerID > 0) {
			jo = factory.createCondition("party_id", " = ", customerID);
			ja.add(jo);
		}

		if (!currency.isEmpty() && !currency.equalsIgnoreCase("null")) {
			jo = factory.createCondition("transactional_curr_code", " = ", currency);
			ja.add(jo);
		}
