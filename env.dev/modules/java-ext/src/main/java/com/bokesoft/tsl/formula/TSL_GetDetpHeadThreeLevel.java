...
public class TSL_GetDetpHeadThreeLevel implements ISelRule {
	...
	public Set<Long> getOpteratorSet(OAContext oaContext, SelRule selRule, Set<Long> opteratorSet, DataTable businessDt,
		...
		if (!ignoreNodeKey.isEmpty()) {
			...
			for (String nodeKey : ignoreNodeKeys) {
		...
		for (int i = 0; i < count; ++i) {
			...
			id = TSL_DepartmentHeadTools.getDeptHead(context, deptID, selRule, businessDt, id, ignoreOperators);
			if (id <= 0) {
				break;
			}
		}
