...
var PagerView = function(id){
	...
	this.render = function(){
		...
		str += '<div class="totalPage"><span>共<i class="number">' + self.itemCount + '</i>条</span></div>';
		...
		if(self._pageCount > 1){
			if(self.index != self._pageCount){
				...
				str += '<a href="javascript://' + self._pageCount + '"><span>末页</span></a>';
			}else{
				str += '<span>下一页</span>';
				str += '<span>末页</span>';
			}
		...
		str +='<span>共<i>' + self._pageCount + '</i>页</span>';
		
		/**
		 * 每页显示指定数量
		 */
		str +='<div class="howItems"><span>每页</span>';
		str +='<select id="'+self.id+'.selectsize">';
		for(var i=0; i<perPageSize.length; i++){
		if(perPageSize[i] == self.size)
		...
		for(var i=0; i<a_list.length; i++){
		 ...
		 var selects = self.container.getElementsByTagName("select");
            (function(fn) {
                var el = selects[0];
