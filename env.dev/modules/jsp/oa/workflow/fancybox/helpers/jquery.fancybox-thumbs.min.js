(function($){var F=$.fancybox;F.helpers.thumbs={wrap:null,list:null,width:0,source:function(item){var href;if(item.element)href=$(item.element).find("img").attr("src");if(!href&&item.type==="image"&&item.href)href=item.href;return href},init:function(opts,obj){var that=this,list,thumbWidth=opts.width||50,thumbHeight=opts.height||50,thumbSource=opts.source||this.source;list="";for(var n=0;n<obj.group.length;n++)list+='<li><a style="width:'+thumbWidth+"px;height:"+thumbHeight+'px;" href="javascript:jQuery.fancybox.jumpto('+
n+');"></a></li>';this.wrap=$('<div id="fancybox-thumbs"></div>').addClass(opts.position||"bottom").appendTo("body");this.list=$("<ul>"+list+"</ul>").appendTo(this.wrap);$.each(obj.group,function(i){var href=thumbSource(obj.group[i]);if(!href)return;$("<img />").load(function(){var width=this.width,height=this.height,widthRatio,heightRatio,parent;if(!that.list||!width||!height)return;widthRatio=width/thumbWidth;heightRatio=height/thumbHeight;parent=that.list.children().eq(i).find("a");if(widthRatio>=
