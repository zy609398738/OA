...
String encodeStr = request.getParameter("typecode");
String decodeStr = URLDecoder.decode(encodeStr, "utf-8");
String _ENTRY_EXP = "ExtraValueSet(EntryFlag,2)+ExtraValueSet(typecode,{-1})+RunUIOpt(New)";
...
try{
	...
	String sql = "SELECT ID FROM CP_OA_NewsType WHERE CODE ="+decodeStr;
	BKRowSet rs = ctx.getResultSet(sql);	
	if(rs.bkFirst()){
		typeID = rs.bkGetInt("ID");
	}
	_ENTRY_EXP = "ExtraValueSet(EntryFlag,2)+ExtraValueSet(typecode,{"+typeID+"})+RunUIOpt(New)+SetValue(NewsTypeID,ExtraValue(typecode))";
}finally{
	if (null!=ctx){
		ctx.close();
	}
