...
<!-- 
用于加载 Yigo 单据的 JSP 页面，来自 “宏信设备” 项目;

主要 HTTP 请求参数
 - title: 初始的页面标题;
 - command: 执行的命令, 可以使用如下命令:
  * 执行菜单入口 - entry:<菜单中定义的 “command” 属性>, "entry:"也可以省略;
  * 显示单据 - openbill:<MetaKey>,<BillID>;
 - entry-exp: 入口公式;

其他 HTTP 请求参数
 - headerFooterHeight: 页面 header/footer 区域的总高度, 根据这个高度系统可以自动调整单据的显示高度;

测试(假设 HTTP 端口为 8080, pages 目录被部署到目录 /Yigo/web2-ext )
 - http://localhost:8080/Yigo/web2-ext/test/Test-HX-DefaultWebView-Operator.jsp
 -->
...
<%!
private String getPara(javax.servlet.http.HttpServletRequest request, String name){
...
%>
<%
String _TITLE = getPara(request, "title");
String _COMMAND = getPara(request, "command");
String _ENTRY_EXP = getPara(request, "entry-exp");
...
//如果是 openbill，检查单据权限
if (_COMMAND.startsWith("openbill:")){
	String metaKey = _COMMAND.substring("openbill:".length(), _COMMAND.indexOf(","));
	IMidContext ctx = null;
	try{
		MidContextFactory fac = new MidContextFactory();
		ctx = fac.createMidContext(request, response);
		
		Env env = (Env) ctx.getEnv();
		int roleId = env.getRoleID();
		if (roleId != 1){	//If not admin, check the Menu right
			String roleList = env.getRoleList();
			if (roleId<0){ //未登录
				roleList = roleId+"";
			}
			String sql =
					"SELECT COUNT(*) AS CNT FROM SCM_METARIGHTS"
				   +" WHERE META_TYPE='AllMenuRight' AND META_KEY LIKE '%,"+metaKey.toUpperCase()+"'"
				   +"   AND META_NR_ROLE IN ("+roleList+")";
			BKRowSet rs = ctx.getResultSet(sql);
			if (rs.bkFirst() && rs.bkGetInt("CNT")>0){
				//有权限
			}else{
				response.getWriter().println("当前用户没有权限打开单据 ["+metaKey+"]");
				return;
			}
		}
	}finally{
...
$(function(){
    ...
    if (! height){
        ...
        <%
        String hfHeight = getPara(request, "headerFooterHeight");
        if (null==hfHeight){
        	%>height = pageHeight - $(divSelector).offset().top - 24/*考虑到常用的 padding 所预留的高度*/;<%
        }else{
        	%>height = pageHeight - <%=hfHeight%>;<%
        }
        %>
    }
    if (height < 600){
