...
Ext.define('Ext.calendar.App', {
	...
	showYigoEditWindow : function(rec, animateTarget) {
		...
		$(border).html("");
		if(rec){
			var title=$("<h2>").html("主题："+rec.data.Title);
			var time=$("<span class='schedule-time'>").html("时间："+rec.data.Begin +" &nbsp至&nbsp "+rec.data.End);
			var sum=$("<span class='schedule-sum'>").html("内容："+rec.data.Content);
			var edit_icon=$("<span class='schedit-icon'>");
