...
void function(jq) {
    ...
    $window.resize(function(){
        ...
        wsWidth = Math.max(1004, $window.innerWidth(), $top.outerWidth() || 0) - ($left.outerWidth() || 0) - 16;
        ...
        $workspace.find('#bill_content, #ws_content').each(function(){
            ...
            $this.width(wsWidth - wSiblings - 12);
        });
        ...
        $workspace.find('.doc-list').each(function(){
            ...
            $this.height(wsHeight - hSiblings - 8).width(wsWidth - wSiblings - 8);
        });
    ...
    jq(function(){
		...
		jq('.create_file_btn[href]').loadToWorkspace();
        // 向指定容器 ajax加载内容的链接
		...
		jq('#create-cancel').click(function(){
			...
			$('#issue_document').fancybox({
            width: 590,
            height: 310,
            autoSize: false,
            type: 'inline',
            content: $('#issue_document_types').html()
        });
