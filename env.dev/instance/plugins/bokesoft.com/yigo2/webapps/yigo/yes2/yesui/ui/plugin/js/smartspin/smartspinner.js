(function($) {
    $.fn.extend({
        spinit: function(options) {
            var settings = $.extend({ min: 0, max: 100, initValue: 0, callback: null, stepInc: 1, pageInc: 10, width: 100, height: 30, btnWidth: 10, mask: '' }, options);
...
//                    if (el.val() == settings.mask) el.val(value);
                    ...
                    if (e.which >= 48 && e.which <= 57) {
                    	var v = el.val() + (e.which - 48);
                        var temp = parseFloat(v);
                        if (v.length <= settings.max.toString().length && temp >= settings.min && temp <= settings.max) {
                            value = temp;
                            raiseCallback(value);
                        } else {
                            e.preventDefault();
