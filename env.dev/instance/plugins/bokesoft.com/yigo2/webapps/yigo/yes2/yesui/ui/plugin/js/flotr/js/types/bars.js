...
Flotr.addType('bars', {
  ...
  hit : function (options) {
    var
      ...
      horizontal = options.horizontal,
      seriesOffset = options.grouped ? (options.barWidth / this.groups) * (options.seriesIndex - this.groups/2 +0.5) : 0,
      geometry, i;

    for (i = data.length; i--;) {
      geometry = this.getBarGeometry(data[i][0] + (horizontal?0:seriesOffset), data[i][1]+ (horizontal?seriesOffset:0), options);
      if (
        // Height:
        (
          // Positive Bars:
  ...
  drawHit : function (options) {
    ...
    var
      ...
      args        = options.args,
      horizontal = options.horizontal,
      seriesOffset = options.grouped ? (options.barWidth / this.groups) * (options.seriesIndex - Math.round(this.groups/2)) : 0,
      geometry    = this.getBarGeometry(args.x + (horizontal?0:seriesOffset), args.y + (horizontal?seriesOffset:0), options),
      left        = geometry.left,
      top         = geometry.top,
      width       = geometry.width,
      height      = geometry.height;
