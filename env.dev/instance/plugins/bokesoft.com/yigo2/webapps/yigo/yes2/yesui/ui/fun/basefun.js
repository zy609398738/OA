UI.BaseFuns = (function () {
    ...
    funs.ChangePWD = function (name, cxt, args) {
        ...
        var rsa = new RSAKey();
        Svr.SvrMgr.getPublicKey().done(function(publicKey) {
            ...
            var paras = {
                ...
                cmd: "ChangePWD",
                operatorID: YIUI.TypeConvertor.toString(operatorID),
                password: password,
                newPassword: newPassword
            };
            Svr.Request.getData(paras);
        });
    };
    funs.New = function (name, cxt, args) {
        ...
        if (target != YIUI.FormTarget.SELF) {
            var params = {formKey: formKey, /*cmd: "PureNewForm", */operationState: YIUI.Form_OperationState.New};
            if (tsParas) {
            ...
            Svr.SvrMgr.getEmptyForm(params).done(function(data) {
                cxt.target = target;
                cxt.optType = YIUI.OptType.NEW;
    ...
    funs.Open = function (name, cxt, args) {
        ...
        var params = {
            formKey: formKey,
            oid: OID.toString(),
            operationState: YIUI.Form_OperationState.Default
        };
        if (tsParas) {
            tsParas = splitPara(tsParas);
            for (var key in tsParas) {
...
//		form.showDocument();
    ...
    funs.Cancel = function (name, cxt, args) {
        ...
        if (form.getOperationState() == YIUI.Form_OperationState.New) {
            ...
            YIUI.UIUtil.showDocument(form);
            form.resetUIStatus(YIUI.FormUIStatusMask.ENABLE | YIUI.FormUIStatusMask.OPERATION);
        }
    };
    ...
    funs.OpenDict = function (name, cxt, args) {
        ...
        Svr.SvrMgr.getEmptyForm(params).done(function(data) {
        	cxt.optType = YIUI.OptType.LOAD;
            YIUI.UIUtil.show(data, cxt, false, YIUI.ShowType.Dict);
    ...
    var doMap = function (name, cxt, args) {
        ...
        var params = {formKey: tgFormKey, operationState: YIUI.Form_OperationState.New}; 
        Svr.SvrMgr.getEmptyForm(params).done(function(data) {
        	cxt.optType = YIUI.OptType.NEW;
            newForm = YIUI.UIUtil.show(data, cxt, false);
            allDoMap(copyDoc, tgFormKey, form, mapKey, newForm, args);
        });
    };
    
    var allDoMap = function (copyDoc, tgFormKey, Form, mapKey, newForm, args) {
    	var form = Form,
        ...
        Svr.SvrMgr.doMapEvent(params).done(function(jsonObj) {
        	...
        	for (var i = 0; i<ignoreKeys.length; i++) {
        		var key = ignoreKeys[i];
        		var cmp = newForm.getMetaComp(key);
        		if (!cmp) {
        			cmp = newForm.getMetaCell(key);
        		}
        		
        		if (cmp) {
        			var tableKey = cmp.tableKey;
        			var colKey = cmp.columnKey;
        			var list = map.get(tableKey);
        			if (!list) {
        				list = new Array();
        	...
        	var goalDocument = newForm.getDocument();//目标表单的document
        	var  tbls = document.tbls;
        	for(var i = 0; i < tbls.length; i++){//下推结果
        		var goalTable = goalDocument.get(i);
        		goalTable.first();
        		//头表数据合并
        		if (goalTable.tableMode == 0){
        			var table = document.get(i);
        			table.first();
            		var rowCount = goalTable.cols.length;
            		
            		var list = map.get(table.key);
            		for (var j = 0; j < rowCount; j++){//目标表单
            			var colKey = goalTable.cols[j].getKey();
            			var col = table.getColByKey(colKey)
            			if(col && list.indexOf(colKey) > -1){
            				goalTable.setByKey(colKey, table.getColByKey(colKey));
            				var setVal = table.get(j); 
            				var type = col.type;
            				if ((type == YIUI.DataType.DATETIME || type == YIUI.DataType.DATE) && setVal != null) {
            					setVal = setVal.getTime();
            				}
            				goalTable.set(j, setVal);
            			}	
            		}
            		
        		}
        		//明细表数据合并
        		if (goalTable.tableMode == 1){
        			var datailTable = tbls[i];
        			if (datailTable != null){
        				var datailRows = datailTable.rows;
        				var goalTabRows = goalTable.rows;
        				for (var i = 0; i < datailRows.length; i++){
        					var goalRowLength = goalTable.rows.length;
        					goalTable.addRow();
        					goalTable.rows[goalRowLength].vals = datailRows[i].vals;
        				}
        			}	
        		}
        	}
        	...
        	newForm.setDocument(goalDocument);
        	...
        	if (mapWorkitemInfo) {
        		...
        		if (info != null){
        			goalDocument.putExpData(YIUI.BPMKeys.SaveBPMMap_KEY, info.WorkitemID);
            		goalDocument.expDataType[YIUI.BPMKeys.SaveBPMMap_KEY] = YIUI.ExpandDataType.LONG;
        		}	
        	}
        	if (args.length > 3) {
    ...
    funs.ShowModal = function (name, cxt, args) {
        ...
        Svr.SvrMgr.getEmptyForm(paras).done(function(data) {
            ...
            var newForm = YIUI.UIUtil.show(data, cxt, false);
            ...
            $.each(columnInfo, function (i, column) {
                ...
                if (column.columnType == YIUI.CONTROLTYPE.COMBOBOX
                    ...
                    $.each(compList, function (i, comp) {
                        if (column.columnKey && comp.columnKey == column.columnKey) {
                            text = comp.getText();
                            value = comp.getValue();
                            exists = true;
                            return;
    ...
    funs.GetExpandValue = function(name,cxt,args){
        ...
        var metaCellInfo = grid.getMetaObj().rows[metaRowIndex].cells[colIndex];
...
//			    form.uiProcess.doCellValueChanged(grid, rowIndex, colIndex, cellKey);
    ...
    funs.GetGroupValue = function (name, cxt, args) {
        ...
        var paras = {
                service: "GetGroupValue",
                formKey: formKey,
                groupKey: groupKey
            };
        var paraGroups = Svr.Request.getSyncData(Svr.SvrMgr.ServletURL, paras);
        var items = null;
        if (paraGroups) {
            items = paraGroups.items;
        }
        return items;
    };
    ...
    funs.StatusValue = function (name, cxt, args) {
        ...
        var paras = {
                service: "GetStatusValue",
                formKey: formKey,
                statusKey: statusKey
            };
        var value = Svr.Request.getSyncData(Svr.SvrMgr.ServletURL, paras)
...
//    };
    ...
    funs.ExportExcel = function (name, cxt, args) {
		...
		if (needDownload) {
			...
			location.href = Svr.SvrMgr.AttachURL + "?formKey=" + form.formKey + "&filePath=" + filePath + "&fileMD5=" + fileMD5 + "&mode=1&service=DownloadExcel";
		}
		return true;
    ...
    {  //BPM函数
        ...
        funs.RestartInstanceByOID = function (name, cxt, args) {
			...
			if (args.length > 0) {
				...
				form.getImplParas().setHostUIStatusProxy(null);
        ...
        funs.OpenWorkitem = function (name, cxt, args) {
			...
			var info = Svr.Request.getSyncData(Svr.SvrMgr.ServletURL, {
				workitemID: WID,
			...
			if (existsAttachment) {
				...
				if (info.AttachmentOperateType == YIUI.AttachmentOperateType.NEW && OID < 0) {
		            Svr.SvrMgr.getEmptyForm({formKey: formKey}).done(function(data) {
		                cxt.optType = YIUI.OptType.NEW;
		                var newForm = YIUI.UIUtil.show(data, cxt);
						if (!onlyOpen) {
							newForm.setSysExpandValue(BPMConstants.WORKITEM_INFO, info);
						}
						// 代表新界面由代办页面打开
						newForm.setSysExpandValue(YIUI.BPMConstants.WORKITEM_VIEW, BPMConstants.WORKITEM_VIEW);
						var doc = newForm.getDocument();
			...
			Svr.SvrMgr.getEmptyForm({formKey: formKey}).done(function(data) {
    			...
    			if (!onlyOpen) {
    				newForm.setSysExpVals(YIUI.BPMConstants.WORKITEM_INFO, info);
    			}
    			// 代表新界面由代办页面打开
    			newForm.setSysExpVals(YIUI.BPMConstants.WORKITEM_VIEW, YIUI.BPMConstants.WORKITEM_VIEW);
    			var filterMap = newForm.getFilterMap();
    			filterMap.setOID(OID);
...
//                YIUI.FormBuilder.diff(form, resultJson);
    ...
    }
    {
        ...
        var submit = function (btn, fileID, tableKey, path, cxt, quickSave) {
            var self = this;
            var form = cxt.form;
            var tgtTable = form.getDocument().getByKey(tableKey);
            var parentTable = form.getDocument().getByKey(tgtTable.parentKey);
            var poid;
            if( parentTable != null ) {
            ...
            console.log(form.OID);
            var enable = btn[0].enable;
            $.ajaxFileUpload({
                url: Svr.SvrMgr.AttachURL,
                secureuri: false,
                fileElement: btn,
                data: {service: "UploadAttachment", formKey: form.formKey, operatorID: $.cookie("userID"), fileID:
                    fileID,poid:poid,quickSave:quickSave, oid: form.getOID(), tableKey: tableKey, path: path, mode: 1},
                type: "post",
                success: function (data, status, newElement) {
                    ...
                    if (table.first()) { // 如果成功上传,那么刷新附件控件
                        ...
                        var viewRow = row.bkmkRow;
                        if( !viewRow ) {
                            tgtTable.addRow(true);
                            var viewRow = new YIUI.DetailRowBkmk();
                            viewRow.setBookmark(tgtTable.getBkmk());
                            row.bkmkRow = viewRow;
                        }
                        tgtTable.setByBkmk(viewRow.getBookmark());
                        for(var i = 0,size = table.cols.length;i < size;i++){
                            ...
                            if((type == YIUI.DataType.DATETIME || type == YIUI.DataType.DATE) && value != null) {
                            	value = value.getTime();
                            }
                            tgtTable.setByKey(colInfo.key,value == undefined ? null : value);
                        }
                        // 子明细父子关联
                        if( parentTable != null ) {
                            tgtTable.setParentBkmk(parentTable.getBkmk());
                        }
                        // 设置当前行状态
                        if( quickSave ) {
                            tgtTable.setState(DataDef.R_Normal);
                        }
        ...
        funs.UploadAttachment = function (name, cxt, args) {
            ...
            target.change(function (e) {
                ...
                submit(target, fileOID, tableKey, path, cxt, quickSave);
            });
        ...
        funs.DownloadAttachment = function (name, cxt, args) {
            ...
            var savePath = "";
            if (args.length > 2) {
                savePath = YIUI.TypeConvertor.toString(args[2]);
            }
            ...
            location.href = Svr.SvrMgr.AttachURL + "?fileID=" + fileOID + "&formKey=" + form.formKey + "&tableKey=" + tableKey
                + "&path=" + path + "&savePath=" + savePath + "&mode=1&service=DownloadAttachment";
            return true;
        };
