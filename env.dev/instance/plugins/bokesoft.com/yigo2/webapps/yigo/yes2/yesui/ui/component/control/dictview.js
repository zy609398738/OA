...
//                            }
							...
							if(!$(node).is(":hidden") && !self.isMultiExpand) {
								$(node).click();
								if(self._selectItem) {
...
//        self.focusNode(curID);
	...
	refreshNode: function(itemData){
        ...
        if (this.isChainDict()) {
            ...
            //删除原来的节点
        	this.removeNode(itemData.itemKey + '_' + itemData.oid);
            //获取当前节点所有父节点
            YIUI.DictService.getParentPath(this.itemKey, itemData).then(function(parents) {
            	
            	if(parents.length > 0) {
            		self.isMultiExpand = true;
            		self._expand(parents[0], curID, 1);
            	}
...
//            self.focusNode(curID);
	...
	dependedValueChange: function(dependedField){
        ...
        var itemKey = this.itemKey;
        var key = this.key;
        var filter = this.handler.getDictFilter(formID, key, itemKey, itemFilters);
