...
YIUI.Control.Grid = YIUI.extend(YIUI.Control, {
    ...
    init: function (options) {
        ...
        this.pageInfo = {
            curPageIndex: 0,
            totalRowCount: 0,
            pageLoadType: this.getMetaObj().pageLoadType,
            pageRowCount: this.getMetaObj().pageRowCount,
            pageCount: 1
        };
    },
    ...
    initOneRow: function (row) {
        ...
        if (row.rowType == undefined) {
            ...
            if (row.metaRowIndex == undefined || row.metaRowIndex == -1) {
                row.metaRowIndex = this.getMetaObj().detailMetaRowIndex;
            }
        }
        row.isDetail = (row.rowType == "Detail");
        row.isEditable = (row.rowType == "Fix" || row.rowType == "Detail");
        row.cellKeys = this.getMetaObj().rows[row.metaRowIndex].cellKeys;
        row.rowHeight = this.getMetaObj().rows[row.metaRowIndex].rowHeight;
    ...
    //             var rowID = modifyRow["rowID"], rowInd, oldRd, mdCell, rn = this.getMetaObj().showRowHead ? 1 : 0;
    //             for (var j = 0, dlen = this.dataModel.data.length; j < dlen; j++) {
    //                 var row = this.dataModel.data[j];
    //                 var curRowID = row["rowID"];
    //                 if (curRowID == rowID) {
    ...
    createCellEditor2: function (cell, rowIndex, colIndex, opt) {
        ...
        editOpt = opt.editOpt, rowID = rowData.rowID;

        var meta = editOpt.editOptions;
        var cellType = meta.cellType;

        //TODO label应该不需要走到这
        if (cellType === YIUI.CONTROLTYPE.LABEL) return;
        ...
        if (cellType == YIUI.CONTROLTYPE.DYNAMIC) {
            ...
            if (cellTypeDef != null) {
                ...
                cellType = cellTypeDef.options.edittype;
        ...
        editor.saveCell = function (value) {
            ...
            //self.el.yGrid("saveCell", iRow, iCol, value);
            //self.el[0].selecting = false;
        };  
        ...
        editor.doFocusOut = function(){
            ...
            // self.yGrid("restoreCell", iRow, iCol);
        };
    ...
    //编辑单元格时如果是自定义编辑组件，则这里进行对应组件的创建
    // createCellEditor: function (cell, iRow, iCol, opt, editOpt) {
    ...
    //             editor = new YIUI.CellEditor.CellComboBox(form, editOpt);
    //             editor.saveCell = function (value) {
    //                 self.getControlGrid().setValueAt(rowIndex, colIndex, value, true, true);
    //                 //self.el.yGrid("saveCell", iRow, iCol, value);
    //                 //self.el[0].selecting = false;
    ...
    //             // self.gridHandler.doGridCellSelect(self, rowID, colIndex);
    //             // var cxt = new View.Context(form);
    //             // cxt.setRowIndex(self.getRowIndexByID(rowID));
    //             // cxt.setColIndex(colIndex);
    //             // opt = $.extend(true, opt, {
    //             //     ofFormID: self.ofFormID,
    //             //     hiding: hideDropView,
    ...
    alwaysShowCellEditor: function (cell, iRow, iCol, cm, value, opt, rowHeight) {
        ...
        switch (editOpt.editOptions.cellType) {
            ...
            case YIUI.CONTROLTYPE.CHECKBOX:
                ...
                editor.click(function (e) {
                    grid.setCellFocus(iRow - 1, grid.getMetaObj().showRowHead ? iCol - 1 : iCol);
    ...
    initPageOpts: function (needCalc) {
        ...
        if (this.getMetaObj().pageLoadType == YIUI.PageLoadType.DB) {
            totalRowCount = YIUI.TotalRowCountUtil.getRowCount(doc, this.tableKey);
        }else{
            totalRowCount = dataTable == null ? 0 : dataTable.size();
        }
        ...
        // var currentPage = this.pageInfo.curPageIndex + 1;
...
//        };
    ...
    onSelectRow: function (iRow, rid) {
        ...
        if (grid.focusRowChanged && iRow != oldRow && iRow >= 0) {
    ...
    getFocusRowIndex: function () {
        ...
        if(!$.isDefined(this.el[0].p.selectRow))
            return -1;
        
        return parseInt($.ygrid.stripPref($.ygrid.uidPref, this.el[0].p.selectRow));
    },
    getFocusColIndex: function () {
        if(this.el == null) return -1;
    ...
    treeIconClick: function (rowdata, iRow) {
        ...
        if (!rowdata.isExpand) {
            var len = childIDs == null ? 0 : childIDs.length;
            for (i = len - 1; i >= 0; i--) {
                cid = childIDs[i];
    ...
    initOptions: function () {
        this.options = {
            ...
            rowSequences: this.getMetaObj().showRowHead,
        ...
        if (this.getMetaObj().treeType !== undefined && this.getMetaObj().treeType !== -1) {
            ...
            if (this.treeExpand) {
                for (i = 0, len = this.treeOldData.length; i < len; i++) {
                    ...
                    if (rd.childRows != null && rd.childRows.length > 0) {
                        rd.isExpand = true;
                    }
    ...
    initGroupHeaders: function () {
        this.el && this.el.initGroupHeaders(this.groupHeaders);
    },
    ...
    //     this.el && this.el.setErrorCells(cells);
    // },
    // setGridErrorRows: function (rows) {
    //     this.el && this.el.setErrorRows(rows);
    // },
...
YIUI.Control.Grid = YIUI.extend(YIUI.Control.Grid, {   //纯web使用的一些方法
        ...
        if( this.getMetaObj().treeType == -1 && !this.hasRowExpand ) {
            ...
            if ( enable && this.getDetailMetaRow() && this.newEmptyRow ) {
                ...
                if ( this.getMetaObj().hideGroup4Editing ) {
                    for (var ri = this.dataModel.data.length - 1; ri >= 0; ri--) {
                        var row = this.dataModel.data[ri];
    ...
    setColumnVisible: function (colKey, visible) {
        ...
        for (ci = 0, clen = this.dataModel.colModel.columns.length; ci < clen; ci++) {
            ...
            if (columnKey === colKey) {
                ...
                colM.hidden = hidden;
                if (this.el) {
                    gridCi = this.getMetaObj().showRowHead ? ci + 1 : ci;
    ...
    setValueAt: function (rowIndex, colIndex, newValue, commitValue, fireEvent, ignoreChanged) {
        ...
        if(isChanged && !ignoreChanged){
            ...
            _this.el[0].modifyGridCell2(rowIndex, colIndex, row, false)
        }
    },
    ...
    addGridRow: function (rowIndex, metaRow, bookmarkRow, groupLevel, fireEvent) {
        ...
        ts.insertGridRow(newRowIndex, rowData);

        var fire = !fireEvent ? true : fireEvent;
        if ( rowData.rowType === 'Detail' && fire ) {
            var form = YIUI.FormStack.getForm(this.ofFormID);
            form.getUIProcess().doPostInsertRow(this, newRowIndex, YIUI.GridUtil.isEmptyRow(rowData));
        }
        ...
        var createNewGroup = function(groupLevel,groupRow){
            ...
            for( var i = 0,size = groupRow.objectArray.length; i < size;i++ ) {
                ...
                if( rowObject.objectType === YIUI.MetaGridRowObjectType.ROW ) {
                    ...
                    var rowData = _this.addGridRow(rowIndex,metaRow,null,groupLevel,true);
    ...
    removeAutoRowAndGroup: function () {
        ...
        for (var i = data.length - 1; i >= 0; i--) {
            row = data[i];
            if (row.inAutoGroup || (row.isDetail && !row.bkmkRow)) {
                data.splice(i, 1);
    ...
    getCellNeedValue: function (rowIndex, colIndex, value, isData) {
        ...
        switch (cellType) {
            ...
            case YIUI.CONTROLTYPE.DICT:
                ...
                if (opt.isDynamic) {
                    var cxt = new View.Context(form);
                    opt.itemKey = form.eval(opt.refKey, cxt);
                }
    ...
    repaintHeaders: function () {
        ...
        for (var ci = 0, clen = this.dataModel.colModel.columns.length; ci < clen; ci++) {
            column = this.dataModel.colModel.columns[ci];
            colHeader = $rowHeader[0].childNodes[ci + (this.getMetaObj().showRowHead ? 1 : 0)];
    ...
    refreshGrid: function (opt) {
        ...
        if (this.getMetaObj().pageLoadType != YIUI.PageLoadType.NONE || this.el[0].p.rowNum == 0) {
    ...
    // addGridRows: function (addJSON, addTable, fireEvent) {
    ...
    //         this.addGridRow(addRow, insertRi + 1, false, fireEvent);
    ...
    //                 this.setValueAt(rowIndex, i, table.getByKey(editOpt.columnKey), false, false, true);
    //             }
    //         } else if (cellType == YIUI.CONTROLTYPE.LABEL 
    //                  || cellType == YIUI.CONTROLTYPE.BUTTON 
    //                  || cellType == YIUI.CONTROLTYPE.HYPERLINK) {
    ...
    setCellFocus: function (rowIndex, colIndex) {
        this.el.setCellFocus(rowIndex + 1, this.getMetaObj().showRowHead ? colIndex + 1 : colIndex);
    },
    ...
    checkHeadSelect: function (colIndex) {
        var hTable = $(this.el[0].grid.hDiv).find(".ui-ygrid-htable"), selectAll = true, hasDataRow;
        for (var i = 0, len = this.getRowCount(); i < len; i++) {
            var rowData = this.getRowDataAt(i);
            if (!YIUI.GridUtil.isEmptyRow(rowData)) {
                hasDataRow = true;
                if ( !rowData.data[colIndex][0] ) {
                    selectAll = false;
                    break;
                }
            }
        }
        $("input", hTable)[0].checked = hasDataRow ? selectAll : false;
    },
