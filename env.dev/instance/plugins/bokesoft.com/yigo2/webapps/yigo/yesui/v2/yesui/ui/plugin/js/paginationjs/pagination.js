    ...
    $.fn[pluginName] = function(options){
        ...
        var pagination = {
            initialize: function(){
                ...
                var el = $("<div class='paginationjs-pages'></div>").hide();
                model.el = el;
...
//                	model.isFirstRender = false;
            ...
            createTemplate: function(args){
                ...
                html += '</ul>';
            	if(showPageDetail) {
            		html += '<div class="'+ classPrefix +'-nav"> '+YIUI.I18N.pagination.joint+'' + totalNumber + ''+YIUI.I18N.pagination.totalRecord+' </div>';
            	}
        		if(showGoInput) {
        			html += '<div class="'+ classPrefix +'-go-input"><input type="text" class="paginationjs-go-pagenumber"/>/'+totalPage+''+YIUI.I18N.pagination.page+'</div>'
        		}
        		if(showGoButton) {
...
//                container.trigger(eventPrefix + 'go', Math.min(attributes.pageNumber, self.model.totalPage));
                self.go(Math.min(attributes.pageNumber, self.model.totalPage));
            }
        };
