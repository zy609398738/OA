YIUI.Control.Grid = YIUI.extend(YIUI.Control, {
    ...
    alwaysShowCellEditor: function (cell, iRow, iCol, cm, value, opt, rowHeight) {
        ...
        opt.click = function () {
            ...
            if (grid.selectFieldIndex == editOpt.colIndex) {
...
YIUI.Control.Grid = YIUI.extend(YIUI.Control.Grid, {   //纯web使用的一些方法
    ...
    setFocusRowIndex: function (rowIndex) {
        if (rowIndex < 0 || rowIndex >= this.dataModel.data.length)
            return;
        var autoScroll = false, timeout = null, set = true;
        var rowTop = rowIndex * this.rowHeight, grid = this, scrollTop = this.el[0].grid.bDiv.scrollTop;
        var visibleHeight = scrollTop + this.el[0].grid.bDiv.offsetHeight;
        if (rowTop < scrollTop || rowTop > visibleHeight) {
            this.el[0].grid.bDiv.scrollTop = rowTop - this.rowHeight;
            autoScroll = true;
        }
        var clickFistCell = function () {
            if (!set) return;
            var gr = grid.el.getGridRowById($.ygrid.uidPref + rowIndex), cellIndex = grid.el[0].p.rowSequences ? 1 : 0;
            var cell = gr.cells[cellIndex];
            $(cell).click();
            set = false;
        };
        $(this.el[0].grid.bDiv).scroll(function () {
            if (timeout != null) {
