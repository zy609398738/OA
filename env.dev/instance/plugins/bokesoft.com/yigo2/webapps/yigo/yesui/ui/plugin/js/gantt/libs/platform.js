...
(function($){
    ...
    function openBlackPopup (url, width, height, onCloseCallBack, iframeId) {
        ...
        bdiv.append("<img src=\"yesui/ui/plugin/js/gantt/res/closeBig.png\" style='cursor:pointer;position:absolute;right:-40px;top:30px;'>");
    ...
    //contextPath 目前没有具体用处，暂时保留
    var contextPath;
    $.extend(YIUI.GANTT, {
        // transform string values to printable: \n in <br>
        transformToPrintable: function(data){
        ...
        isValidInteger: function (n) {
            var reg = new RegExp("^[-+]{0,1}[0-9]*$");
            return reg.test(n);
        },

        isValidDouble: function(n) {
        ...
        isValidTime: function(n) {
            return !isNaN(this.millisFromHourMinute(n));
        },

        isValidDurationDays: function (n) {
            return !isNaN(this.daysFromString(n));
        },


        isValidDurationMillis: function (n) {
            return !isNaN(this.millisFromString(n));
        },

        /*
         supports almost all Java currency format e.g.: ###,##0.00EUR   €#,###.00  #,###.00€  -$#,###.00  $-#,###.00
         */
        isValidCurrency: function(numStr) {
        ...
        getMillisInHours: function (millis) {
            if (!millis)
                return "";
        ...
        getMillisInHoursMinutes: function (millis) {
            ...
            return  (sgn>0?"":"-")+ this.pad(hour,1,"0") + ":" + this.pad(min,2,"0");
        },

        getMillisInDaysHoursMinutes: function(millis) {
            if (!millis)
                return "";
            ...
            return (sgn>=0?"":"-")+(days > 0 ? days + "  " : "") + this.pad(hour,1,"0") + ":" + this.pad(min,2,"0");
        },

        millisFromHourMinute: function( stringHourMinutes ) { //All this format are valid: "12:58" "13.75"  "63635676000" (this is already in milliseconds)
            var result = 0;
         ...
         */
        millisFromString: function(string, considerWorkingdays) {
            if (!string)
                return 0;
                ...
                for (var i = 1; i < matcher.length; i++) {
                    ...
                    if (match) {
                        ...
                        if (i == 1) { // years
                            ...
                            totMillis = totMillis + this.millisFromHourMinute(match);
                        } else if (i == 9) { // :minutes
                            totMillis = totMillis + this.millisFromHourMinute(match);
         ...
         * @return milliseconds. 0 if invalid string
         */
        daysFromString: function(string, considerWorkingdays) {
            if (!string)
                return undefined;
        ...
        validateField: function(ev) {
            ...
            if (value) {
                ...
                if (type == "INTEGER") {
                    rett = this.isValidInteger(value);
                } else if (type == "DOUBLE") {
                    rett = this.isValidDouble(value);
                } else if (type == "PERCENTILE") {
                    rett = this.isValidDouble(value);
                } else if (type == "URL") {
                    rett = this.isValidURL(value);
                } else if (type == "EMAIL") {
                    rett = this.isValidEmail(value);
                } else if (type == "DURATIONMILLIS") {
                    rett = this.isValidDurationMillis(value);
                } else if (type == "DURATIONDAYS") {
                    rett = this.isValidDurationDays(value);
                } else if (type == "DATE") {
                    rett = Date.isValid(value,el.attr("format"));
                } else if (type == "TIME") {
                    rett = this.isValidTime(value);
                } else if (type == "CURRENCY") {
                    rett = this.isValidCurrency(value);
                }
        ...
        jsonErrorHandling: function( response ) {
            if (!response.ok){
                if (response.message)
                    alert("ERROR:\n"+ response.message);
        ...
        createBlackPage: function(width, height, onCloseCallBack) {
            if (!width)
                width='900px';
            if (!height)
                height='730px';
            ...
            var bg=$("<div>").attr("id","__blackpopup__");
            bg.css({position:'fixed',top:"0px",paddingTop:"50px", left:0,width:'100%',height:'100%',  backgroundImage:"url('yesui/ui/plugin/js/gantt/res/black_70.png')"});
            ...
            var img=$("<img src='yesui/ui/plugin/js/gantt/res/closeBig.png' style='cursor:pointer;position:absolute;right:-40px;top:5px;' title='close'>");
            bdiv.append(img);
            img.click( function(){
                bg.trigger("close");
            });
        ...
        closeBlackPopup: function (){
            getBlackPopup().trigger("close");
        },
        //------------------------------------------------ TEAMWORK SPECIFIC FUNCTIONS   --------------------------------------------------------
        openIssueEditorInBlack: function (issueId, command, params) {
            if (!command)
                command="ED";
        ...
        openBoardInBlack: function(boardId, command, params, callback) {
            if (!command)
                command="ED";
    ...
    $.fn.createErrorAlert = function(errorCode, message) {
        this.each(function() {
            var el = $(this);
