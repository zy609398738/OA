...
var define;
var richEditor = YIUI.I18N.richEditor;
...
$.extend($E, {
    ...
    'styleConfig': {
        'fontFamilyOptions': [
            richEditor.arial, richEditor.blackbody, richEditor.regularScript, 
            richEditor.officialScript, richEditor.youyuan, richEditor.microsoftYaHei, 
            'Arial', 'Verdana', 'Georgia', 'Times New Roman', 'Microsoft JhengHei',
            'Trebuchet MS', 'Courier New', 'Impact', 'Comic Sans MS'
        ],
        'colorOptions': {
        	'#880000': richEditor.marroon,
        	'#800080': richEditor.purple,
        	'#ff0000': richEditor.red,
            '#ff00ff': richEditor.brightPink,
            '#000080': richEditor.mazarine,
            '#0000ff': richEditor.blue,
            '#00ffff': richEditor.lakeBlue,
            '#008080': richEditor.blushGreen,
            '#008000': richEditor.green,
            '#808000': richEditor.olive,
            '#00ff00': richEditor.reseda,
            '#ffcc00': richEditor.aurantius,
            '#808080': richEditor.gray,
            '#c0c0c0': richEditor.silver,
            '#000000': richEditor.black,
            '#ffffff': richEditor.white
        },
        'fontsizeOptions': {
            1: '10px',
            2: '13px',
    ...
    'demoAlertConfig': {
        insertExpression: {
            title: richEditor.insertExpression,
            isAlert: false
        },
        insertImage: {
            title: richEditor.insertImage,
            isAlert: false
        },
        insertCode: {
            title: richEditor.insertCode,
            isAlert: false
        }
    }
});
	    ...
	    $uploadImg_modal.find('#' + btnId).click(function(e){
	    	 ...
	    	 uploadImg_callback = function(){
	            ...
	            var file = $('#' + fileId)[0].files[0];
	                 ...
	                 editor.command(e, 'insertImage', this.result, uploadImg_callback);
    ...
    'createMenu': function(menu, editor){
        ...
        //弹出框
            ...
            btnClick = function(e){
                ...
                var mtop = 0;
                if (txtTop + $modal.height() >= $(window).height()) {
                	$modal.css('margin-top', txtTop - modalContainerTop + 100 - (($modal.offset().top + $modal.height())-$(window).height()));
                } else {
                	
                	$modal.css('margin-top', txtTop - modalContainerTop + 5);
                }
                 if ($modal.hasClass('wangEditor-modal-big')) {
                	 $('textarea',$modal).val(editor.latestHtml);
                 }
...
//        });
        ...
        .on('mousedown', function(){
            ...
            editor.$txtContainer.addClass("wangEditor-textarea-focus")
            ...
            //focus blur 时记录，以便撤销
            editor.$txtContainer.removeClass("wangEditor-textarea-focus")
    ...
    'modal': function (editor) {
        ...
        $(function(){
            ...
            if(uploadImgComponent){  
                //如果有上传组件
                //将上传组件移动到model中
            	//$uploadImg_modal.append(uploadImgComponent);
...
$.extend($E.fn, {
	'commandHooks':{
        ...
        'fullScreen': function(commandName, commandValue){
            ...
            if(position !== 'fixed'){
                //记录_editorContainer高度
                this._editorContainerHeight = $editorContainer.height();
                this._editorContainerWidth = $editorContainer.width();
                //记录$txtContainer的宽高
                this._txtContainerHeight = $txtContainer.height();
                this._txtContainerWidth = $txtContainer.width();
                //修改_editorContainer高度
                $editorContainer.css({
                    'height': '90%',
                    'width':'98%'
                });
                ...
                //去掉maxheight
                $editorContainer.css('max-height', 'none');

            }else{
                //还原txtContainer高度
            	$editorContainer.height(this._editorContainerHeight);
            	$editorContainer.width(this._editorContainerWidth);
            	
            	$txtContainer.height(this._txtContainerHeight);
            	$txtContainer.width(this._txtContainerWidth);
                //还原
                $editorContainer.css({
                    'position': 'relative',
                    'top': 0,
                ...
                //还原maxheight
                $editorContainer.css('max-height', maxHeight);
            }
        }
    }
});
