		...
		var Return = {
			...
			initSearchBox: function() {
				...
				ZoomControl.prototype.initialize = function(map){
					...
					// 添加文字说明
					var input = $("<input class='search_txt' placeholder="+YIUI.I18N.baidumap.inputPlace+"/>")
					div.append(input);
			...
			addMarker: function(point, name, icon) {
				...
				marker.addEventListener("click", function(e) {
					var opts = {
					  	width : 200,     // 信息窗口宽度
				  		height: 100,     // 信息窗口高度
				  		title : YIUI.I18N.baidumap.eventTest // 信息窗口标题
					}
					var infoWindow = new BMap.InfoWindow(YIUI.I18N.baidumap.address, opts);  // 创建信息窗口对象 
			...
			addMenuItem: function(overlay) {
				...
				markerMenu.addItem(new BMap.MenuItem(YIUI.I18N.attachment.ldelete,removeMarker.bind(overlay)));
			...
			clearBtn: function() {
				...
				a.attr("title", YIUI.I18N.grid.eliminate);
				var self = this;
				a.click(function() {
					self.clear();
		            self.drawMgr.close();
			...
			boundaryBtn: function() {
				var a = $("<a class='BMapLib_box BMapLib_boundary' title="+YIUI.I18N.baidumap.canton+"/>");
				var self = this;
				var map = this.map;
				var div = $(".BMapLib_Drawing_panel", options.el);
				a.click(function() {
					if($(".Boundary_Box").length == 0) {
						...
						var input = $("<input type='text' class='txt' placeholder="+YIUI.I18N.baidumap.inputName+" />");
						var btn = $("<input type='button' class='btn' value="+YIUI.I18N.dict.determine+" />");
						...
						var searchFun = function(e) {
							...
							bdary.get(val, function(rs){       //获取行政区域
								...
								if (count === 0) {
									alert(YIUI.I18N.baidumap.failTo);
									return ;
								}
					          	var pointArray = [];
				...
				if (path.length > 0) {
					for (var i = 0, len = path.length; i < len; i++) {
						...
						if(len == 1 || i == len - 1) {
							current = point;
							var marker = new BMap.Marker(point);
					  		var label = new BMap.Label(YIUI.I18N.baidumap.currentDot, {offset: new BMap.Size(20,-10)});
							marker.setLabel(label);
