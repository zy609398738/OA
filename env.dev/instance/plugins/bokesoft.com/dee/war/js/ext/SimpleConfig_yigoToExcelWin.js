 ...
 * 配置的window
 */
function yigoToExcelWin(text, description, record, detailId, detailType,
		newtablename, hidden) {
	var win_buttons = [{
		text : '保存',
		handler : function() {
			...
			Ext.Ajax.request({
				url : 'interfaceYigoToExcelController.do?actionType=updateSdInterfaceYigoToExcel',
				params : {
	...
	if (record && record.billTable) {
		...
		getYigoToExcelTabpanelConfigItems(win_form, record);
	}
 ...
 */
function chooseYigoToExcelColumns(win_form, json) {
	var mapData = json;
	var win_buttons = [{
		text : '确定',
		handler : function() {
			...
			if (tableNames.length != 0) {
				...
				getYigoToExcelTabpanelConfigItems(win_form, mapData, result);
			}
			win.close();
...
function getYigoToExcelTabpanelConfigItems(win, map, selected) {
	...
	for (var i = 0; i < tableNames.length; i++) {
		if (Ext.Array.contains(keys, tableNames[i])) {// 如果表的tab已经有了，只需要把store加入就行
			...
			var win_form_Config_grid = Ext.create('Ext.grid.Panel', {
				...
				closable : i != 0,
				store : Ext.create('Ext.data.Store', {
							fields : ['colName', 'key', 'caption', 'fieldType',
									'dbColumnName', 'dicTable'],
							data : selected
									? selected[tableNames[i]]
