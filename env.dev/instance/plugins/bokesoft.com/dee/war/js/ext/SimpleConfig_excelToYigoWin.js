 ...
 * 配置的window
 */
function excelToYigoWin(text, description, record, detailId, detailType,
		newtablename, hidden) {
	var win_buttons = [{
		text : '保存',
		handler : function() {
			...
			Ext.Ajax.request({
				url : 'interfaceExcelToYigoController.do?actionType=updateSdInterfaceExcelToYigo',
				params : {
	...
	if (record && record.billTable) {
		...
		getExcelToYigoTabpanelConfigItems(win_form, record);
	}
 ...
 */
function chooseExcelToYigoColumns(win_form, json) {
	var mapData = json;
	var win_buttons = [{
		text : '确定',
		handler : function() {
			...
			if (tableNames.length != 0) {
				...
				getExcelToYigoTabpanelConfigItems(win_form, mapData, result);
			}
			win.close();
...
function getExcelToYigoTabpanelConfigItems(win, map, selected) {
