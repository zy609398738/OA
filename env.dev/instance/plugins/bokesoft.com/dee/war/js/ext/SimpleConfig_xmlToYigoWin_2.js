 ...
 * 配置的window
 */
function xmlToYigoWin_2(text, description, record, detailId, detailType,
		newtablename,hidden) {
	var win_buttons = [{
		text : '保存并下载xml',
		handler : function() {
			...
			var form_billTable=Ext.getCmp('win_form_tabpanel').items.keys.join(',');
			var yigoconfig = {
			...
			Ext.Ajax.request({
				url : 'interfaceSimpleConfigXmlToYigo20Controller.do?actionType=updateSdInterfaceByIdAndType_2',
				params : {
				...
				success : function(response) {
					...
					if (result.result) {
						...
						Ext.Msg.show({
							...
							fn : function(type) {
								if ('yes' == type) {
									window.open("downloadSimpleFileFromString.jsp?filename="
													+ newtablename + ".xml");
								}
							}
						});
	...
	if (record && record.billTable) {
		...
		getTabpanelConfigitems_2(win_form, record);
	}
 ...
 */
function winForchooseTablecolumns_2(win_form, json) {
	var mapData = json;
	var win_buttons = [{
		text : '确定',
		handler : function() {
			...
			if (tableNames.length != 0) {
				...
				getTabpanelConfigitems_2(win_form, mapData, result);
			}
			win.close();
...
//				resizable : false,
	...
	var tabpanelitems = getTabpanelitems_2(mapData);
 ...
 */
function getTabpanelitems_2(map) {
 ...
 */
function getTabpanelConfigitems_2(win, map, selected) {
	...
	for (var i = 0; i < tableNames.length; i++) {
		if (Ext.Array.contains(keys, tableNames[i])) {// 如果表的tab已经有了，只需要把store加入就行
			...
			var win_form_Config_grid = Ext.create('Ext.grid.Panel', {
				...
				closable : i!=0,
				store : Ext.create('Ext.data.Store', {
							fields : ['key', 'caption', 'dataType', 'format',
