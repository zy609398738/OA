 ...
 * 配置的window
 */
function yigoToXmlWin(text, description, record, detailId, detailType,
		newtablename, hidden) {
	var treestore;
	var win_buttons = [{
		text : '保存',
		handler : function() {
			...
			Ext.Ajax.request({
				url : 'interfaceYigoToXmlController.do?actionType=updateSdInterfaceYigoToExcel',
				params : {
											...
											Ext.Ajax.request({
												...
												success : function(response) {
													...
													Ext
															...
															.removeAll();
													getYigoToXmlTabpanelConfigItems2(
															win_form,
															tableNames,
															yigoconfig,
															comboboxtree);
	...
	if (record && record.billTable) {
		...
		getYigoToXmlTabpanelConfigItems(win_form, record, null, treestore);
	}
 ...
 */
function chooseYigoToXmlColumns(win_form, json) {
	var mapData = json;
	var win_buttons = [{
		text : '确定',
		handler : function() {
			...
			if (tableNames.length != 0) {
				...
				getYigoToXmlTabpanelConfigItems(win_form, mapData, result);
			}
			win.close();
...
function getYigoToXmlTabpanelConfigItems(win, map, selected, treestore) {
			...
			var win_form_Config_grid = Ext.create('Ext.grid.Panel', {
				...
				store : Ext.create('Ext.data.Store', {
							fields : ['createOnElement', 'key', 'caption',
									'fieldType', 'dbColumnName', 'dicTable',
									'format'],
							data : selected
									? selected[tableNames[i]]
...
function getYigoToXmlTabpanelConfigItems2(win, tableNames, selected,
		comboboxtree) {
	var result = [];
	var keys = Ext.getCmp('win_form_tabpanel').items.keys;
	for (var i = 0; i < tableNames.length; i++) {
