 ...
 * 
 */
function dstableToYigoWin(text, description, record, detailId, detailType,
		newtablename, hidden) {
	var win_buttons = [{
		text : '保存',
		handler : function() {
			...
			for (var i = 0; i < tableNames.length; i++) {
				var sql = getCmp(tableNames[i]).getValues().midTableSql.Trim();
				if ('' == sql || null == sql) {
					Ext.Msg.alert('提示', '中间表到[' + tableNames[i]
									+ ']的查询语句不能为空！！！');
					return;
				} else {
					if (i != 0 &&(sql.toUpperCase().trim().indexOf('SELECT')==0)&&sql.trim().split(' ').length!=1&& sql.indexOf('#[') == -1) {// 子表的查询语句必须关联主表
						Ext.Msg.alert('提示', '中间表到[' + tableNames[i]
					...
					yigoconfig[tableNames[i] + '_store'] = storeToObj(Ext
							.getCmp('win_form_tableToYigo' + tableNames[i]).store);
				}
			}
			var mapData = {// 第一个map
				id : detailId,
			...
			Ext.Ajax.request({
				url : 'interfaceSimpleConfigTableToYigoController.do?actionType=updateSdInterfaceTableToYigo',
				params : {
	...
	if (record && record.billTable) {
		...
		getTableToYigoTabpanelConfigItems(win_form, record);
	}
	var win = Ext.create('Ext.Window', {
				title : 'table2Yigo配置[' + newtablename + ']',
 ...
 * tabletoyigo 选择表的列名
 */
function chooseTableToYigoColumns(win_form, json) {
	var mapData = json;
	var win_buttons = [{
		text : '确定',
		handler : function() {
			...
			if (tableNames.length != 0) {
				...
				getTableToYigoTabpanelConfigItems(win_form, mapData, result);
			}
			win.close();
...
//				resizable : false,
	...
	var tabpanelitems = getTableToYigoTabpanelItems(mapData);
 ...
 */
function getTableToYigoTabpanelItems(map) {
 ...
 */
function getTableToYigoTabpanelConfigItems(win_form, map, selected) {
	...
	for (var i = 0; i < tableNames.length; i++) {
		if (Ext.Array.contains(keys, tableNames[i])) {// 如果表的tab已经有了，只需要把store加入就行
			// 已有的key不能再加入
			var items = Ext.getCmp('win_form_tableToYigo' + tableNames[i]).store.data.items;
			...
			Ext.getCmp('win_form_tableToYigo' + tableNames[i]).store
			...
			var win_form_Config_grid = Ext.create('Ext.grid.Panel', {
				id : 'win_form_tableToYigo' + tableNames[i],
				store : Ext.create('Ext.data.Store', {
							fields : ['midKey', 'key', 'caption', 'fieldType',
					...
					items : [{
								...
								handler : function() {
									...
									var form_midTableSql = win_form.currentActive.items.items[0].items.items[0].rawValue
											.Trim();
									var firstSql = getCmp('win_form_tabpanel').items.items[0].items.items[0].items.items[0].rawValue.Trim();
									if (null == form_midDataSource
											|| '' == form_midDataSource) {
										Ext.Msg.alert('提示', '请选择中间表数据源！！！');
										return;
									}
									if (null == firstSql || '' == firstSql) {
										Ext.Msg.alert('提示',
												'中间表到主表的查询语句不能为空！！！');
										return;
									}
									if (null == form_midTableSql
											|| '' == form_midTableSql) {
										Ext.Msg.alert('提示', '中间表查询语句不能为空！！！');
										return;
									}
									bodyLoadingMask.show();
									Ext.Ajax.request({
										url : 'interfaceSimpleConfigTableToYigoController.do?actionType=getColumnsBySql',
										params : {
