 ...
 * 
 */
function yigo2TableWin(text, description, record, detailId, detailType,
		newtablename, hidden) {
	var win_buttons = [{
		text : '保存',
		handler : function() {
			...
			Ext.Ajax.request({
				url : 'interfaceSimpleConfigYigoToTableController.do?actionType=updateSdInterfaceTableToYigo',
				params : {
 ...
 * tabletoyigo 选择表的列名
 */
function chooseYigoToTableColumns(win_form, json) {
	var mapData = json;
	var win_buttons = [{
		text : '确定',
		handler : function() {
...
//			result[tableNames[0]] = storeToObj(tabstore);
			...
			if (tableNames.length != 0) {
				...
				getYigoToTableTabpanelConfigItems(win_form, mapData, result);
			}
			win.close();
 ...
 */
function getYigoToTableTabpanelConfigItems(win_form, map, selected) {
	var currentActiveId;
			...
			var win_form_Config_grid = Ext.create('Ext.grid.Panel', {
				...
				store : Ext.create('Ext.data.Store', {
							fields : ['midKey', 'key', 'caption',
									'dbColumnName', 'fieldType', 'dicTable'],
							data : selected
									? selected[tableNames[i]]
									: map[tableNames[i]],
							autoLoad : true
...
function addTableTable(win_form, tablename, dataSourceId, midData) {
	bodyLoadingMask.show();
	Ext.Ajax.request({
		url : 'interfaceSimpleConfigYigoToTableController.do?actionType=getColumnsBySql',
		params : {
			id : dataSourceId,
			sql : tablename
		},
...
//							resizable : false,
				...
				if (!midData) {
					...
					getYigoToTableTabpanelConfigItems(win_form, midData, false);
				}
...
function getYigoInfo(win_form) {
	...
	Ext.Ajax.request({
				...
				success : function(response) {
					...
					var result = Ext.decode(response.responseText);
					if (result.result) {
						chooseYigoToTableColumns(win_form, result.data);
					} else {
						Ext.Msg.alert('失败', result.data);
					}
