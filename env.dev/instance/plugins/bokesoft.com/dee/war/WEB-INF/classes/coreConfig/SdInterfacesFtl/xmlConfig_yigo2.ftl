  ...
  <#if tableName_index=0>
	<${tableName}>
	 <#list freeMarkerData["${tableName}_store"] as itemKey>
		<${itemKey.key}<#if itemKey.dicTable!="">_code</#if>>${cdataStart}<#if itemKey.caption?has_content>${itemKey.caption}</#if>(<#if itemKey.dataType?has_content>type=${itemKey.dataType},</#if><#if itemKey.format?has_content>format=${itemKey.format},</#if><#if itemKey.primaryKey="true">primaryKey=${itemKey.primaryKey},</#if>required=${itemKey.allowBlank})${cdataEnd}</${itemKey.key}<#if itemKey.dicTable!="">_code</#if>>
	 </#list>
	</${tableName}>
  <#else>
	<${tableName}s>
