	...
	<custom-transformer class="${freeMarkerData.className}" name="${freeMarkerData.text}">
		...
		<spring:property name="deployMapping">
				<spring:map>
				<#list freeMarkerData.deployMapping?keys as itemKey>
				 <#if itemKey="name"||itemKey="index">
					<spring:entry key="${itemKey}" value="${freeMarkerData.deployMapping[itemKey]}"/>
				 <#elseif itemKey="cell">
					 <spring:entry key="cell">
					 	<spring:list>
						 	<#assign item = freeMarkerData.deployMapping[itemKey]>
				 ...
				 <#else>
				 	 ...
				 	 <spring:map>
				 	 <#list freeMarkerData.deployMapping[itemKey]?keys as itemKey2>
				 	 	<#if itemKey2="title"||itemKey2="startLine"||itemKey2="endConditions">
				 	 	<spring:entry key="${itemKey2}" value="${freeMarkerData.deployMapping[itemKey][itemKey2]}"/>
				 	 	<#else>
				 	 	<spring:entry key="list">
					 	<spring:list>
						 	<#assign item = freeMarkerData.deployMapping[itemKey][itemKey2]>
