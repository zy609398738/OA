    ...
    Plugin.prototype = {
        _init: function () {
            ...
            for (var i = 0; i < groupLength; i++) {
                ...
                className = iconsGroup[i].className;
                index = 0;
                if (!path || !maxNum) {
            ...
            var emoji_preview = '<div class="emoji_preview"><img src="'+path+'"/></div>';
                    ...
                    if (target === $(btn)[0]) {
                        ...
                        if (code) {
                            if (field.nodeName === 'DIV') {
                                var $img = $('#emoji_container_' + ix + ' a[data-emoji_code="' + code + '"] img');
                                imgSrc = $img.attr('src');
                                imgCls = $img.attr('class');
                                imgCode = $img.data('emoji_code');
                                insertHtml = '<img data-emoji_code="'+imgCode+'" class="emoji_icon '+imgCls+'" src="' + path + '"/>';
            ...
            $('#emoji_container_' + ix + ' .emoji_icons a').mouseenter(function () {
                ...
                var $img = $(this).find('img'),
                    src = $img.attr('src'),
                    cls = $img.attr('class');
                $('#emoji_container_' + ix + ' .emoji_preview img').attr({'src': src,'class':cls}).removeClass('mCS_img_loaded').parent().show();
            });
                ...
                for (var i = 0; i < groupLength; i++) {
                    ...
                    if (alias) {
                        ...
                        that.$content.html(that.$content.html().replace(regexp, function ($0, $1) {
                            var n = revertAlias[$1];
                            if (n) {
                                return '<img class="emoji_icon '+(className && className[n] ? className[n] : '')+' " src="' + path + '"/>';
                            } else {
                                return $0;
                            }
                        }));
                        ...
                        that.$content.html(that.$content.html().replace(new RegExp(pattern, 'gm'), '<img class="emoji_icon '+(className && className[$1] ? className[$1] : '')+'" src="' + path + '"/>'));
                    }
                }
            }
        }
