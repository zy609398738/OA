define(function () {
    ...
    /** make executable, only used in *nix */
    var makeExecutable = function(path){
    	path = _formalPath(path, "makeExecutable");
    	if (! path){
            log.warn("Path is BLANK("+path+"), can't do chmod");
            return;
        }
    	require(["std/os"], function(os){
    		if (os.isWindows){
    			log.warn("Can't set executable permission for Windows file.");
    		}else{
    			var perms = java.nio.file.Files.getPosixFilePermissions(path.toPath());
    			perms.add(java.nio.file.attribute.PosixFilePermission.OWNER_EXECUTE);
    			perms.add(java.nio.file.attribute.PosixFilePermission.GROUP_EXECUTE);
    			perms.add(java.nio.file.attribute.PosixFilePermission.OTHERS_EXECUTE);
    			java.nio.file.Files.setPosixFilePermissions(path.toPath(), perms);
    		}
        });
    }
    
    /** Copy file */
    var cp = function(sourcePath, targetPath){
    	var src = new java.io.File(sourcePath);
    	var tgt = new java.io.File(targetPath);
